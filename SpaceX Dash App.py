...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(values=spacex_df.groupby('Launch Site')['class'].sum(),
            names=spacex_df.groupby('Launch Site')['Launch Site'].first(),
            title='Total Number of Successful Launches by Site')

    else:
        fig = px.pie(values=spacex_df[spacex_df['Launch Site']==str(entered_site)]['class'].value_counts(normalize=True), 
            names=spacex_df['class'].unique(),
            title='Total Number of Successful Launches at {}'.format(entered_site))
    return(fig)
...
def get_scatter_chart(entered_site, payload_mass):
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df[spacex_df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
            ...
            title='Payload Mass vs Success by Launch Site')
    else:
        ...
        fig = px.scatter(filtered_df[filtered_df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
            x="Payload Mass (kg)", y="class",
            color="Booster Version Category",
            title='Payload Mass vs Success for at {}'.format(entered_site))
    return(fig)
